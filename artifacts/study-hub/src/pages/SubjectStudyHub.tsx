...
type LectureInteraction = "normal" | "manage" | "select" | "drag";
...
export function SubjectStudyHub() {
  ...
  const renderLectureCard = (lecture: Lecture, index: number, interaction: LectureInteraction = "normal") => {
  ...
  return <div className="space-y-6 pb-24">
    ...
    <AnimatePresence>{lectureMode === "select" && <motion.div key="lecture-selection-actions" {...actionBarMotion} className="fixed inset-x-4 bottom-20 z-40 mx-auto flex max-w-md gap-3 rounded-3xl border border-border/60 bg-card p-3 shadow-xl md:bottom-8"><button type="button" disabled={!selectedLectures.size} onClick={deleteSelected} className="min-h-12 flex-1 rounded-2xl bg-destructive px-4 font-semibold text-destructive-foreground transition-all duration-200 hover:opacity-90 active:scale-[.98] disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transform-none"><Trash2 className="mr-2 inline h-4 w-4" />Delete</button><button type="button" onClick={cancelSelect} className="min-h-12 flex-1 rounded-2xl border border-border/60 bg-secondary px-4 font-semibold text-foreground transition-all duration-200 hover:bg-secondary/80 active:scale-[.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transform-none">Cancel</button></motion.div>}</AnimatePresence>
