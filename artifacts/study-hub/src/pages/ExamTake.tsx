...
import { ArrowLeft, ChevronLeft, ChevronRight, Clock3, Sparkles, X } from "lucide-react";
...
export function ExamTake() {
  ...
  if (result) {
    ...
    const reviewContent = missed.map(entry => entry.kind === "question" ? <GlassCard key={entry.question.id} className="space-y-3 rounded-3xl p-5"><b>{entry.question.text}</b><p className="text-sm text-emerald-600">Correct: {entry.question.choices[entry.question.correctAnswer - 1]}</p><p className="text-sm text-destructive">Your answer: {entry.question.choices[Number(answers[entry.question.id] ?? 0) - 1] ?? "Unanswered"}</p></GlassCard> : <GlassCard key={entry.card.id} className="rounded-3xl p-4"><FaceContent text={entry.card.front} image={entry.card.frontImage} alt="Question" /><div className="mt-3 border-t border-border/40 pt-4"><FaceContent text={entry.card.back} image={entry.card.backImage} alt="Answer" /></div></GlassCard>);
    return <div className="mx-auto max-w-3xl space-y-5 pb-24"><header className="flex items-center gap-3"><button onClick={goBack} className="flex h-11 w-11 items-center justify-center rounded-full border border-border bg-secondary"><ArrowLeft className="h-5 w-5" /></button><div><p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{result.reviewOnly ? "Review complete" : "Exam complete"}</p><h1 className="text-2xl font-bold">{exam.name}</h1></div></header><ExamResultPanel percentage={result.score.percentage} band={band} correct={result.score.correct} total={result.score.total} flashPoints={result.flashPoints} missedCount={missed.length} reviewOnly={result.reviewOnly} automatic={result.automatic} showMissed={showMissed} onToggleMissed={() => setShowMissed(value => !value)} onRestart={() => restart(false)} onRetryWrong={() => restart(true)} onDone={goBack}>{reviewContent}</ExamResultPanel></div>;
  }
