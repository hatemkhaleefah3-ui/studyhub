...
export function Subjects() {
  ...
  const subjectCard = (subject: OrderedSubject, interaction: "normal" | "manage" | "select" | "drag" = "normal") => {
    ...
    const card = <GlassCard className={`group/card relative flex h-full ${compact ? "min-h-20 p-3" : "min-h-[11rem] p-5"} flex-col overflow-hidden rounded-[1.75rem] border bg-card shadow-sm transition-all duration-200 ease-in-out ${isSelected ? "border-primary ring-2 ring-primary/30 shadow-md" : "border-border/60"}`}>
      <div className="absolute left-5 right-5 top-0 h-1 rounded-b-full" style={{ backgroundColor: subject.color }} />
      ...
      {!compact && <div className="pointer-events-none absolute right-3 top-3 h-20 w-20 rounded-full opacity-[.08] blur-2xl transition-transform duration-300 group-hover/card:scale-110" style={{ backgroundColor: subject.color }} />}
      <div className="relative z-10 flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><motion.div whileHover={{ rotate: -6, scale: 1.08 }} transition={{ type: "spring", stiffness: 420, damping: 22 }} className={`flex ${compact ? "h-11 w-11" : "h-12 w-12"} shrink-0 items-center justify-center rounded-2xl border shadow-sm`} style={{ backgroundColor: `${subject.color}14`, borderColor: `${subject.color}30`, color: subject.color }}><SubjectIcon className={compact ? "h-5 w-5" : "h-6 w-6"} strokeWidth={2.1} /></motion.div><div className="min-w-0"><h3 className={`${compact ? "truncate text-lg" : "line-clamp-2 text-xl"} font-bold leading-tight tracking-tight text-foreground`}>{subject.name}</h3><p className="mt-1 text-xs text-muted-foreground">{subject.lectures.length} lectures · {attachmentCount} attachments</p></div></div>{interaction === "drag" && <GripVertical className="h-7 w-7 shrink-0 text-primary" />}</div>
      {!compact && <div className="relative z-10 mt-auto pt-5"><div className="mb-2 flex items-center justify-between text-xs"><span className="font-semibold text-muted-foreground">Study progress</span><span className="font-bold text-foreground">{progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-secondary"><div className="h-full rounded-full transition-[width] duration-300 ease-in-out motion-reduce:transition-none" style={{ width: `${progress}%`, backgroundColor: subject.color }} /></div><div className="mt-4 flex gap-2"><div className="flex-1 rounded-2xl border border-border/40 bg-secondary/35 px-3 py-2"><span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Lectures</span><b className="ml-2 text-sm">{subject.lectures.length}</b></div><div className="flex-1 rounded-2xl border border-border/40 bg-secondary/35 px-3 py-2"><span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Attachments</span><b className="ml-2 text-sm">{attachmentCount}</b></div></div></div>}
    </GlassCard>;
    if (interaction === "select") return <button type="button" onClick={() => toggle(subject.id)} className="h-full w-full rounded-[1.75rem] text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{card}</button>;
    if (interaction === "manage" || interaction === "drag") return card;
    return <SwipeableRow onEdit={() => openEdit(subject.id)} onDelete={() => setDeletingId(subject.id)} className="h-full"><Link href={`/subjects/${subject.id}`} className="group block h-full rounded-[1.75rem] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="h-full cursor-pointer transition-all duration-200 group-hover:-translate-y-1 group-hover:shadow-xl group-active:scale-[.99] motion-reduce:transform-none">{card}</div></Link></SwipeableRow>;
  };
  ...
  return <div className="space-y-6 pb-24">
    ...
    {mode === "manage" && reorder ? <SortableCardList items={orderedSubjects} onChange={applyOrder} renderItem={subject => subjectCard(subject, "drag")} /> : sortedSubjects.length === 0 ? <GlassCard className="mt-12 flex flex-col items-center justify-center rounded-3xl border-2 border-dashed bg-transparent p-12 text-center"><div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full border border-border/50 bg-secondary/50 text-primary"><BookOpen className="h-7 w-7" /></div><h2 className="mb-2 text-2xl font-semibold tracking-tight">No subjects yet</h2><p className="max-w-md text-muted-foreground">Create your first subject to start organizing your lectures, attachments, and tasks.</p></GlassCard> : <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">{sortedSubjects.map(subject => <div key={subject.id}>{subjectCard(subject, mode === "select" ? "select" : mode === "manage" ? "manage" : "normal")}</div>)}</div>}
